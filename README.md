...
4. 변수, 자료형 - [소스](./day02/ex04_variable.py)
    - 선언이 없고 자료형을 지정안함
    - 자료형 자체를 사용안함, 형변환 필요
    - 기본자료형, int, float, str, bool, None Type(NULL과 거의 똑같은 기능)
...
11. 라이브러리 사용 계속
    ...
    - 파이썬은 자신만의 패키지 관리자(Package Manger : pip) 사용
...
15. 객체지향
    ...
    - C++의 객체지향, 클래스와 동일
    - 접근제한자가 없음(public, privated, protectd) 개발자가 알아서 하라
    - C++과 갈리 new 안 씀, 변수등 선언 계약
    - 클래스 내의 모든 함수의 첫번째 파리미터는 `self`로 시작, C++의 this와 동일
    - 호출시에는 self를 사용 X
    - 파이썬의 철학 : `막지말고`, `알아서 지켜라`
    - public, private(_로 변수 선언), protectd(_ 변수 선언). C++처럼 접근제한자를 많이 사용안함
...
- JSON
    - Javasript Object Notation : 자바스크립트에서 데이터를 사용하기 위해 만든 표기방법
    - 딕셔너리를 텍스트화
...
- 주피터 노트북 단축키
    ...
    - Ctrl + enter : 마크다운셀은 빠져나오기, 코드 셸을 실행
